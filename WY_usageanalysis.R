...
Clan_ConAAfreq_Dfbac <- read.csv( 'Clan_data_ancestralAAC_NQbac.csv', header= T)
Clan_ConAAfreq_Dfarc <- read.csv( 'Clan_data_ancestralAAC_NQarc.csv', header= T)

# SWITCH THE FILES FROM ARCH TO BAC DEPENDING ON WHICH PLOTS YOU WANT TO GENERATE
Pfam_ConAAC <- PfamConAACNQarc
Clan_ConAAfreq_Df <- Clan_ConAAfreq_Dfarc
...
laca_lbca_dfarc <- read.csv( 'laca_lbca_df_arc.csv',header = T)
laca_lbca_dfbac <- read.csv( 'laca_lbca_df_bac.csv',header = T)
# change this depending on whether u want nq.arch or nq.bac plot
laca_lbca_df <- laca_lbca_dfarc
...
W_sitesproblistarc <- readRDS("W_sitesproblistPfamIDs0.4_arc.rds")
W_sitesproblistbac <- readRDS("W_sitesproblistPfamIDs0.4_bac.rds")
W_sitesproblist <- W_sitesproblistarc
...
Y_sitesproblistarc <- readRDS("Y_sitesproblistPfamIDs0.4_arc.rds")
Y_sitesproblistbac <- readRDS("Y_sitesproblistPfamIDs0.4_bac.rds")
Y_sitesproblist <- Y_sitesproblistarc 
...
#write.csv(W_freq_df_ArcBac, 'W_freq_df_ArcBac.csv', row.names = F)
W_freq_df_ArcBac <- read.csv('W_freq_df_ArcBac.csv', header = T) # READ AFTER GENERATING USING LINES ABOVE
