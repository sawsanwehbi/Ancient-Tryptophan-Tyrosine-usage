...
Pfam_ConAAC <- read.csv('../PFAM Trees/Pfam_data_ancestralAAC.csv', header = T)
...
Clan_ConAAfreq_Df <- read.csv('../PFAM Trees/Clan_data_ancestralAAC.csv', header = T)

#Clan_ConAAfreq_Df <- Clan_ConAAfreq_Df[-which(Clan_ConAAfreq_Df$Clans %in% unclassifiableclans),]
unclassifiableclans <-  unique(Pfam_ConAAC$clans[which(Pfam_ConAAC$ancestor == 'unclassifiable')])
unclassifiablePostLUCAclans <- unique(Clan_ConAAfreq_Df$Clans[which(Clan_ConAAfreq_Df$Clan_ancestor == 'postLUCA' & 
                                Clan_ConAAfreq_Df$Clans %in% unclassifiableclans)])
unclassifiableLUCAclans <- unique(Clan_ConAAfreq_Df$Clans[which(Clan_ConAAfreq_Df$Clan_ancestor == 'LUCA' & 
                           Clan_ConAAfreq_Df$Clans %in% unclassifiableclans)])
table(Clan_ConAAfreq_Df$Clan_ancestor[which(Clan_ConAAfreq_Df$Clans %in% unclassifiableclans[which(grepl('CL...', unclassifiableclans))])])
Pfam_ConAAC$pfamIDs[which(Pfam_ConAAC$clans %in% unclassifiablePostLUCAclans &
                            Pfam_ConAAC$ancestor =='unclassifiable')] #PF01571 PF04993, LUCA first then lbca,PF04509, PF13442 lbca then luca
Pfam_ConAAC$pfamIDs[which(Pfam_ConAAC$clans %in% unclassifiableLUCAclans &
                            Pfam_ConAAC$ancestor =='unclassifiable')] #PF01420,PF01590,PF02110, PF04371,PF08308,PF08543,PF18884,PF19571 6 clans
...
#### Ancient Archaea and bacteria usages #####
...
## PostLUCA clans with >1 LACA/LBCA or 1 preLACA/preLBCA; stricter classification
...
#diversifiedLACAclans <- diversifiedLACAclans[-which(diversifiedLACAclans  %in% unclassifiablePostLUCAclans)]#26
...
#diversifiedLBCAclans <- diversifiedLBCAclans[-which(diversifiedLBCAclans %in% unclassifiablePostLUCAclans)]#233
...
DivLACAclan_usage <- colWeightedMeans(as.matrix(Clan_ConAAfreq_Df[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLACAclans ),2:21]),
                    Clan_ConAAfreq_Df$Clan_Conlength[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLACAclans )] )/
                     NoDivpostLUCA_Clans_ConAAC
DivLACAclan_usage <- DivLACAclan_usage[match(  AA_properties$Letter, names(DivLACAclan_usage))]
DivLBCAclan_usage <- colWeightedMeans(as.matrix(Clan_ConAAfreq_Df[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLBCAclans ),2:21]),
...
laca_lbca_df <- cbind( DivLACAclan_usage, DivLBCAclan_usage, DivLACAclanratio_se, DivLBCAclanratio_se)
ggplot( laca_lbca_df,aes(y = as.numeric(DivLACAclan_usage), 
  x = as.numeric(DivLBCAclan_usage), label =rownames(laca_lbca_df))) +
  ...
  theme(legend.position = 'none') + xlim(0.4 ,1.7) + ylim(0.4,1.7) +
...
### Middle nucleotide usages in ancient bacteria vs archaea ####
AncAdenine_clanfreq <- rowSums(Clan_ConAAfreq_Df[,which(colnames(Clan_ConAAfreq_Df) %in% 
   AA_properties$Letter[which(AA_properties$middle_nt_letter=='A')])]*Clan_ConAAfreq_Df$Clan_Conlength)/
  Clan_ConAAfreq_Df$Clan_Conlength
AncUracil_clanfreq <- rowSums(Clan_ConAAfreq_Df[,which(colnames(Clan_ConAAfreq_Df) %in% 
   AA_properties$Letter[which(AA_properties$middle_nt_letter=='U')])]*Clan_ConAAfreq_Df$Clan_Conlength)/
  Clan_ConAAfreq_Df$Clan_Conlength
AncGuanine_clanfreq <- (rowSums(Clan_ConAAfreq_Df[,which(colnames(Clan_ConAAfreq_Df) %in% 
                    AA_properties$Letter[which(AA_properties$middle_nt_letter=='G')])]*Clan_ConAAfreq_Df$Clan_Conlength) +
                          ((1/3)*Clan_ConAAfreq_Df$S*Clan_ConAAfreq_Df$Clan_Conlength))/ Clan_ConAAfreq_Df$Clan_Conlength
AncCytosine_clanfreq <- (rowSums(Clan_ConAAfreq_Df[,which(colnames(Clan_ConAAfreq_Df) %in% 
                           AA_properties$Letter[which(AA_properties$middle_nt_letter=='C')])]*Clan_ConAAfreq_Df$Clan_Conlength) +
                           ((2/3)*Clan_ConAAfreq_Df$S*Clan_ConAAfreq_Df$Clan_Conlength))/ Clan_ConAAfreq_Df$Clan_Conlength

Clan_middlentfreq_Df <- cbind(Clan_ConAAfreq_Df, AncAdenine_clanfreq,
                              AncUracil_clanfreq ,AncGuanine_clanfreq ,AncCytosine_clanfreq )
NoDivpostLUCA_Clans_middlent <- colWeightedMeans(as.matrix(Clan_middlentfreq_Df[which(Clan_middlentfreq_Df$Clan_ancestor == 'postLUCA' &
                            !Clan_middlentfreq_Df$Clans %in% Broaddiversifiedclans),24:27]),
                             Clan_middlentfreq_Df$Clan_Conlength[which(Clan_middlentfreq_Df$Clan_ancestor == 'postLUCA' &
                            !Clan_middlentfreq_Df$Clans %in% Broaddiversifiedclans)] )
DivLACAmiddlent_usage <- colWeightedMeans(as.matrix(Clan_middlentfreq_Df[which(Clan_middlentfreq_Df$Clans %in% diversifiedLACAclans ),24:27]),
                        Clan_middlentfreq_Df$Clan_Conlength[which(Clan_middlentfreq_Df$Clans %in% diversifiedLACAclans )] )/ NoDivpostLUCA_Clans_middlent 
DivLBCAmiddlent_usage <- colWeightedMeans(as.matrix(Clan_middlentfreq_Df[which(Clan_middlentfreq_Df$Clans %in% diversifiedLBCAclans ),24:27]),
                          Clan_middlentfreq_Df$Clan_Conlength[which(Clan_middlentfreq_Df$Clans %in% diversifiedLBCAclans )] )/ NoDivpostLUCA_Clans_middlent 
DivLBCA_LACAmiddlent_usageDf <- data.frame(DivLACAmiddlent_usage,DivLBCAmiddlent_usage )
write.csv(DivLBCA_LACAmiddlent_usageDf ,'DivLBCA_LACAmiddlent_usage.csv')

AncientArchBacvsLUCA <- data.frame(colWeightedMeans(as.matrix(Clan_ConAAfreq_Df[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLACAclans ),2:21]),
                       Clan_ConAAfreq_Df$Clan_Conlength[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLACAclans )] )/LUCA_Clans_ConAAC,
                      colWeightedMeans(as.matrix(Clan_ConAAfreq_Df[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLBCAclans ),2:21]),
                      Clan_ConAAfreq_Df$Clan_Conlength[which(Clan_ConAAfreq_Df$Clans %in% diversifiedLBCAclans )] )/LUCA_Clans_ConAAC)
colnames(AncientArchBacvsLUCA ) <- c('Ancient Archaea usage','Ancient Bacteria usage')
ggplot(AncientArchBacvsLUCA, aes(y = as.numeric(`Ancient Archaea usage`), 
  x = as.numeric(`Ancient Bacteria usage`), label = rownames(AncientArchBacvsLUCA))) + 
  ylab('Ancient Archaea usage') + xlab('Ancient Bacteria usage')  +
  geom_hline(yintercept = 1, linetype = 2) + geom_vline(xintercept = 1,linetype = 2) +
  geom_text(size=8) + theme(axis.text=element_text(size=24),axis.title=element_text(size=32,face="bold"),
  legend.text=element_text(size=25), legend.title = element_text(size=25,face="bold"))
...
DivLACAclanratio_var <-  (DivLACAweightedse[-1]^2)/((NoDivpostLUCA_Clans_ConAAC)^2) +
  (NoDivpostLUCAweightedse[-1]^2)*((DivLACA_ClanAAC)^2)/((NoDivpostLUCA_Clans_ConAAC)^4)
...
DivLBCAclanratio_var <-  (DivLBCAweightedse[-1]^2)/((NoDivpostLUCA_Clans_ConAAC)^2) +
  (NoDivpostLUCAweightedse[-1]^2)*((DivLBCA_ClanAAC)^2)/((NoDivpostLUCA_Clans_ConAAC)^4)
...
StrictNumNoDivAA_properties <- cbind( NoDiv_LUCAusage,NodivLUCAclanratio_se, NoDiv_preLUCAusage,NodivpreLUCAclanratio_se ,
                            DivLACAclan_usage, DivLBCAclan_usage, DivLACAclanratio_se, DivLBCAclanratio_se)
#colnames(StrictNumNoDivAA_properties) <- paste0('strict_',colnames(StrictNumNoDivAA_properties) )
write.csv(StrictNumNoDivAA_properties, '../Tryptophan paper/StrictNoDivAA_properties.csv')

summary(lm(AA_properties$Filtered_avg_2000 ~ NoDiv_LUCAusage , weights = 1 / (AA_properties$Filtered_sd_2000^2) ))
summary(lm(NoDiv_LUCAusage ~ AA_properties$MolecularWeightsDa , weights = 1/(NodivLUCAclanratio_se^2) ))
summary(lm(NoDiv_preLUCAusage ~ AA_properties$MolecularWeightsDa , weights = 1/(NodivpreLUCAclanratio_se^2) ))
...
Pfam_ConAAC[Pfam_ConAAC$pfamIDs == 'PF14349',]
Pfam_ConAAC$ancestor[Pfam_ConAAC$clans == 'CL0193']
table(Clan_ConAAfreq_Df$Clan_ancestor[which(Clan_ConAAfreq_Df$Clans %in%
              unique(Pfam_ConAAC$clans[which(Pfam_ConAAC$pfamIDs %in% tmhmm_alpha_pfams)]))])

# luca is luca+preluca & post-luca is post-luca+modern
...
trans_chisq_table <- data.frame(c(0,2), c(92,699), row.names = c('LUCA', 'post-LUCA'))
colnames(trans_chisq_table ) <- c('Beta sheets', 'Alpha helices')
chisq.test(trans_chisq_table)
# PF00133 W freq is 3.1% contemporary and 3.9% ancestral
